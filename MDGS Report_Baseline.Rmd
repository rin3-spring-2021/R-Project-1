...
```{r school types stacked bar chart}
...
ggplot(data = public_subtypes,
       mapping = aes(x = school_type_detail,
                     y = n)) +
  geom_col() +
  coord_flip()
...
```{r locale type cond public}

# Create summary table of the data

public_locales2 = all_schools %>% 
  mutate(locale_label_cond = 
           case_when(locale_label %in% c("City large", "City small") ~ "Urban"
                     locale_label %in% c("Rural distant", ),
                     TRUE ~ NA) %>% 
  filter(school_type == "public") %>% 
